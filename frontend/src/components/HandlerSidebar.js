...
const sidebarCss = `
  ...
  .pg-handler-tag { font-size: 10px; background: #eef2f7; color: var(--purple); padding: 2px 8px; border-radius: 10px; font-weight: 700; letter-spacing: 0.05em; }
...
export default function HandlerSidebar({ collapsed, onToggle, hidden }) {
  const pathname = usePathname();
  const router = useRouter();
  const [userName, setUserName] = useState("");
  const [userRole, setUserRole] = useState("handler");
  ...
  useEffect(() => {
    setUserName(localStorage.getItem("userName") || "Handler");
    setUserRole(localStorage.getItem("userRole") || "handler");
    
    const savedCollapse = localStorage.getItem("handlerSidebarCollapsed") === "true";
  ...
  const toggleSidebar = () => {
    ...
    localStorage.setItem("handlerSidebarCollapsed", newVal ? "true" : "false");
  };
  ...
  const renderSidebarContent = () => (
    <>
      <div className="pg-logo-wrap relative">
        <Link href="/handler/dashboard" onClick={() => setDrawerOpen(false)} className="pg-logo-row" style={{ paddingRight: "30px" }}>
        ...
        <span className="pg-handler-tag">HANDLER PANEL</span>
    ...
    <>
      ...
      <AnimatePresence>
        {drawerOpen && (
          ...
          >
            {/* Dedicated Floating Close Button */}
            <button
              type="button"
              onClick={(e) => {
                e.stopPropagation();
